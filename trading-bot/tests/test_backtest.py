...
from tradingbot.strategy import StrategyParams, make_adapter
...
def _default_adapter():
    ...
    return make_adapter(strategy_params)
...
def test_flat_prices_produce_no_trades():
    ...
    result = run_backtest(
        {"FLAT/USDT": df}, _default_adapter(), _default_risk(),
...
def test_sustained_uptrend_produces_profitable_long():
    ...
    result = run_backtest(
        {"UP/USDT": df}, _default_adapter(), _default_risk(),
...
def test_sustained_downtrend_produces_profitable_short():
    ...
    result = run_backtest(
        {"DOWN/USDT": df}, _default_adapter(), _default_risk(),
        starting_equity=10_000, fee_rate=0.0004, slippage_bps=5,
    )
...
def test_higher_fees_reduce_total_pnl():
    ...
    cheap = run_backtest(
        {"UP/USDT": df.copy()}, _default_adapter(), _default_risk(),
        starting_equity=10_000, fee_rate=0.0, slippage_bps=0,
    )
...
def test_risk_per_trade_scales_position_size():
    ...
    result_high = run_backtest(
        {"UP/USDT": df.copy()}, _default_adapter(), risk_high,
        starting_equity=10_000, fee_rate=0.0, slippage_bps=0,
    )
